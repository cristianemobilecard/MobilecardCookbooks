...
node[:deploy].each do |application, deploy|
  ...
  webapp_dir = ::File.join(node['tomcat7']['webapps_base_dir'], deploy[:document_root].blank? ? application : deploy[:document_root])
  ...
  ruby_block "remove unnecessary directory entries in #{current_dir}" do
    block do
      node['tomcat7']['webapps_dir_entries_to_delete'].each do |dir_entry|
  ...
  execute 'trigger tomcat service restart' do
    command '/bin/true'
    not_if { node['tomcat7']['auto_deploy'].to_s == 'true' }
    notifies :restart, resources(:service => 'tomcat')
  end
end

include_recipe 'tomcat7::context'
